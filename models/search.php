...
if(isset($_POST['global-search']) && check_token($referer, "search", "post")){
    ...
    if($_SESSION['search_count'] <= ((((time()-$_SESSION['search_time'])/60)+1)*$search_limit)){
        ...
        if(!empty($results)){
            ...
            $msg_error .= "Nenhum resultado encontrado. Verifique os termos pesquisados (Mais de 3 caracteres) ou tente outras palavras."."<br>";
        }
    }else{
        unset($_SESSION['search_results']);
...
<span typeof="v:Breadcrumb"><span property="v:title"><?php echo $texts['SEARCH']; ?></span></span> </div>
</div>
